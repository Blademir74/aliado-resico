...
function fiscalEngine(message) {
  ...
  if (t.includes('anual')) { respuestaFiscal = 'Declaración anual: EXENTA si solo tienes ingresos RESICO, no superas $3.5M y cumpliste pagos mensuales. OBLIGATORIA si tuviste salarios > $400,000, intereses reales > $100,000, dividendos o ingresos mixtos (Art. 113-F LISR).'; fundamentoLegal = 'Art. 113-F LISR'; accionConcreta = 'Usa el Wizard Fiscal para determinar tu obligación con tus montos exactos.'; }
  else if (t.includes('buzón') || t.includes('buzon')) { respuestaFiscal = 'El Buzón Tributario es obligatorio para RESICO. Inactivo genera multa de $3,420 a $10,260 MXN y se duplica por reincidencia.'; fundamentoLegal = 'Art. 17-K y 86-C CFF'; accionConcreta = 'Actívalo hoy en sat.gob.mx → Mi Portal → Buzón Tributario.'; }
  else if (t.includes('e.firma') || t.includes('efirma') || t.includes('firma')) { respuestaFiscal = 'La e.firma tiene vigencia de 4 años. Sin ella no puedes timbrar CFDI ni presentar declaraciones.'; fundamentoLegal = 'Art. 17-D CFF'; accionConcreta = 'Sube tu .cer a Mi Carpeta Fiscal para calcular tus días restantes.'; }
  else if (t.includes('gasto') || t.includes('ticket') || t.includes('deducir') || t.includes('iva')) { respuestaFiscal = 'En RESICO los gastos NO reducen tu ISR (tributas sobre ingresos brutos), pero el IVA desglosado SÍ es acreditable si el gasto es estrictamente indispensable y tienes CFDI válido.'; fundamentoLegal = 'Art. 113-E LISR · Arts. 5 y 29 Ley del IVA'; accionConcreta = 'Sube el ticket al OCR y guárdalo en tu Bóveda de Evidencia IVA.'; }
  else if (t.includes('límite') || t.includes('limite') || t.includes('3.5') || t.includes('expul')) { respuestaFiscal = 'El límite RESICO 2026 es $3,500,000 MXN anuales. Al excederlo, el SAT te migra a Actividad Empresarial con tasas hasta 35%.'; fundamentoLegal = 'Art. 113-E LISR'; accionConcreta = 'Vigila el semáforo: 80% preventivo, 90% alto, 94% expulsión.'; }
  else { respuestaFiscal = 'En RESICO 2026 el ISR se calcula sobre ingresos efectivamente cobrados con tasas anuales: hasta $300,000 → 1.0%; $600,000 → 1.1%; $1,000,000 → 1.5%; $2,500,000 → 2.0%; $3,500,000 → 2.5%.'; accionConcreta = 'Pregúntame por: declaración anual, buzón tributario, e.firma, gastos e IVA, o límite de ingresos.'; }
...
async function callVertex(prompt) {
  ...
  const r = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ contents: [{ role: 'user', parts: [{ text: prompt }] }] }) });
...
async function callAIStudio(model, prompt) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${GEMINI_KEY}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }] }) });
