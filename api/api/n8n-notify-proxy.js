...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Validación de forma: solo payloads de alerta fiscal
    ...
    if (!N8N_URL) {
      return res.status(200).json({ ok: true, forwarded: false, reason: 'n8n_no_configurado', engine: ENGINE });
    }
    // Firma HMAC para que n8n verifique el origen (anti-spoofing)
    ...
    if (!upstream.ok) {
      return res.status(200).json({ ok: true, forwarded: false, reason: `n8n_http_${upstream.status}`, engine: ENGINE });
    }
    return res.status(200).json({ ok: true, forwarded: true, engine: ENGINE });
  } catch (e) {
    return res.status(200).json({ ok: true, forwarded: false, reason: 'exception', detail: String(e?.message || ''), engine: ENGINE });
  }
}