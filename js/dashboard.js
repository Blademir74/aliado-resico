/* ============================================
   ALIADO RESICO — Dashboard v5.2
   Fix: usa APP_STATE.supabase (no window.supabase)
   Fix: IDs reales del DOM (no #income-alert-container)
   Fix: maybeSingle() — no lanza si fila no existe
   Fix: render inmediato desde Store (sin esperar red)
   ============================================ */
const Dashboard = (() => {
  ...
  function _level(income) {
    ...
    if (income >= L.MAX) return {
      ...
      msg: `<strong>LÍMITE REBASADO — $${fmt(income)} MXN.</strong> Expulsión automática al Régimen General. Tasas hasta 35%.`,
      ref: 'Art. 113-E, fracc. III, LISR 2024',
    };
    if (income >= L.A94) return {
      cls: 'critical', pct, badge: `🔴 CRÍTICO ${pct.toFixed(1)}%`,
      msg: `<strong>ALERTA ROJA — $${fmt(income)} MXN.</strong> Margen: $${fmt(L.MAX-income)} MXN. Detén operaciones facturadas.`,
      ref: 'Art. 113-E, fracc. III, LISR 2024',
    };
    if (income >= L.A90) return {
      cls: 'high', pct, badge: `🟠 RIESGO ${pct.toFixed(1)}%`,
      msg: `<strong>ALERTA NARANJA — $${fmt(income)} MXN.</strong> Margen: $${fmt(L.MAX-income)} MXN. Consulta tu proyección.`,
      ref: 'Art. 113-E, fracc. III, LISR 2024',
    };
    ...
    return {
      ...
      msg: `Ingresos dentro del límite. Margen: <strong>$${fmt(L.MAX-income)} MXN</strong>.`,
      ref: 'Art. 113-E, fracc. III, LISR 2024',
    };
  }
  ...
  function renderIncomeMonitor(income = 0) {
    ...
    if (msg)   { msg.className = `alert-message ${a.cls}`; msg.innerHTML = a.msg + `<span class="alert-ref">${a.ref}</span>`; }
  }
  ...
  function renderHealth(buzon, efirma) {
    ...
    if (!buzon && box) {
      ...
      if (mEl) mEl.textContent = 'Buzón tributario inactivo. Multa: $10,260 MXN. Por reincidencia: $20,520 MXN.';
  ...
  function renderFeed(items = []) {
    ...
    if (!list) return;
    if (!items.length) { list.innerHTML = '<p style="color:var(--text-muted);padding:1rem 0">Sin actividad reciente.</p>'; return; }
  ...
  // FIX: cliente desde APP_STATE.supabase (no window.supabase)
  async function syncAndRender() {
    // Render inmediato desde Store local — cero latencia de red
    if (window.Store) {
    ...
    try {
      ...
      if (met) {
        ...
        renderKPIs({ totalProcessed: met.total_processed||0, avgConfidence: Math.round((met.avg_confidence||0)*100), autoResolutionRate:0, avgResponseTime:2.3 });
        Store?.updateIncome?.(met.income_ytd || 0);
      }
      if (convs) renderFeed(convs);

    } catch(err) { console.warn('[Dashboard] sync:', err.message); }
  }

  // Alias para compatibilidad con app.js v3.1 que llama Dashboard.render()
