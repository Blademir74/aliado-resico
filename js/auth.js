...
// FIX CRÍTICO: _runCombinedValidation() restaurada (faltaba su
// declaración completa; solo quedaba el punto de llamada, lo cual
// rompía initialize() → init() → App.init() con ReferenceError).
// ============================================================
...
`;
  ...
  function _injectWelcomeMessage() {
    ...
    bubble.innerHTML =
      ...
      `Tu acceso fue validado y la Bóveda Fiscal quedó sincronizada. ` +
  ...
  // ── FIX CRÍTICO: función restaurada ─────────────────────────
  // Valida sesión activa de Supabase + acceso RLS a fiscal_metrics.
  // Esta función existía solo como llamada huérfana en initialize();
  // su declaración completa se había perdido, causando el
  // ReferenceError que bloqueaba TODO el login (real y en cascada
  // el flujo de App.init()).
