// ════════════════════════════════════════════════════════════
// js/ocr.js — OCR Multimodal v9.0 (CFDI 4.0 + Art. 27 Fracc. III LISR)
// Precisión objetivo: 97% · Safety Flag: 85%
// ════════════════════════════════════════════════════════════
const DocumentProcessor = (() => {
  ...
  // ── Compresión client-side (FIX 413: Vercel <4.5MB) ──────────────────
  ...
  // ── VALIDACIÓN CFDI 4.0 (Art. 29-A CFF) ──────────────────────────────
  // Reglas críticas: razón social SIN régimen, CP exacto, uso compatible
  ...
  function validateCFDI40(data, perfil) {
    ...
    // 1. Razón social SIN régimen societario
    const razon = String(data.razon_social_receptor || data.nombre_receptor || '').toUpperCase();
    if (razon && REGIMEN_TOKENS.test(razon)) {
      warnings.push(`⚠️ CFDI 4.0: La Razón Social del receptor incluye régimen societario ("${razon}"). Debe coincidir EXACTO con tu Constancia (sin "SA DE CV").`);
      hardFail = true;
    }
    
    // 2. Código Postal exacto del domicilio fiscal
    const cp = String(data.cp_receptor || '').trim();
    if (perfil?.cp && cp && cp !== String(perfil.cp).trim()) {
      warnings.push(`⚠️ CFDI 4.0: El CP del receptor (${cp}) no coincide con tu domicilio fiscal registrado (${perfil.cp}).`);
      hardFail = true;
    }
    
    // 3. Uso del CFDI compatible con régimen
    const uso = String(data.uso_cfdi || '').toUpperCase();
    if (uso && !RESICO_USOS_OK.includes(uso)) {
      warnings.push(`⚠️ CFDI 4.0: Uso "${uso}" incompatible con régimen RESICO (626).`);
      hardFail = true;
    }
  ...
  function renderResult(payload) {
    ...
    const reviewBadge = safety.needs_review
      ? `<span style="background:#f59e0b;color:#000;padding:3px 10px;border-radius:6px;font-size:12px;font-weight:700;">
           ⚠️ Verificación Humana Requerida (${safety.confidence_pct}% confianza)
         </span>`
      : `<span style="background:#10b981;color:#fff;padding:3px 10px;border-radius:6px;font-size:12px;font-weight:700;">
           ✅ Validado IA (${safety.confidence_pct}% confianza)
         </span>`;

    const noteBlock = fiscalNote.applies
      ? `<div style="margin-top:10px;padding:10px;background:rgba(16,185,129,0.1);border-left:3px solid #10b981;border-radius:4px;font-size:13px;">
  ...
  async function analyzeFile(file) {
    ...
    // MODO DEMO: OCR simulado sin backend
    if (window.APP_STATE?.isDemo) {
      await new Promise(r => setTimeout(r, 1200));
      const simulated = {
        document: {
          file_name: file.name, document_type: 'TICKET', confidence: 0.93, source: 'demo_simulado',
          extracted_data: {
            rfc_emisor: 'XAXX010101000', rfc_receptor: 'DEMO123456XXX',
            nombre_emisor: 'COMERCIAL DEMO SA', fecha: new Date().toISOString().slice(0, 10),
            folio: 'DEMO-0001', subtotal: 1000, iva: 160, total: 1160, tax_usefulness: 'IVA'
          }
        }
      };
    ...
    try {
      ...
      if (base64Data.length > 3500000) {
        throw new Error('Imagen demasiado pesada. Toma la foto con mejor iluminación y sin zoom.');
      }
      ...
      const response = await fetch('/api/document-ocr', {
        ...
        body: JSON.stringify({
          ...
          base64Data,
          extractionSchema: {
            rfc_emisor: 'string', rfc_receptor: 'string', razon_social_receptor: 'string',
            cp_receptor: 'string', uso_cfdi: 'string', metodo_pago: 'string', forma_pago: 'string',
            concepto: 'string', nombre_emisor: 'string', fecha: 'YYYY-MM-DD', folio: 'string',
            subtotal: 'number', descuento: 'number', iva: 'number', total: 'number',
            tipo_comprobante: 'CFDI | TICKET | EFIRMA | CONSTANCIA | OPINION | OTRO',
            tax_usefulness: 'ISR | IVA | AMBOS | NINGUNO', tipo_servicio: 'string'
          },
          instructions:
            'Extrae datos fiscales con precisión 97%. ' +
            'CFDI 4.0: reporta razon_social_receptor SIN régimen societario (ej. "EMPRESA PATITO" no "EMPRESA PATITO SA DE CV"). ' +
            'Reporta cp_receptor y uso_cfdi exactamente como aparecen. ' +
            ...
            'Regla Art. 27 Fracc. III LISR: gasolina en efectivo (forma_pago 01) NO es deducible ni acreditable.'
        })
      });
      ...
      // VALIDACIÓN CFDI 4.0 post-procesamiento
      ...
      if (!cfdiCheck.ok) {
        payload.document.safety_flag = true;
        payload.document.needs_review = true;
        payload.document.validation_status = 'CFDI_4.0_invalido';
      }

      // Tratamiento de gastos PF vs PM
      const treatment = window.Store?.computeGastoTreatment?.(payload.document);
      if (treatment) {
        extractedData.gasto_acreditable = treatment.gasto_acreditable;
        extractedData.isr_deducible = treatment.isr_deducible;
      }

      if (output) output.innerHTML = renderResult(payload);

      // Agregar warnings CFDI 4.0 al output
      if (output && cfdiCheck.warnings.length) {
        output.innerHTML += `<div style="margin-top:8px;padding:10px;background:rgba(239,68,68,0.12);border-left:3px solid #ef4444;border-radius:4px;font-size:13px;color:#fecaca;">
          ${cfdiCheck.warnings.map(w => esc(w)).join('<br>')}
        </div>`;
      }

      // Agregar microcopy de tratamiento de gastos
      if (output && treatment) {
        output.innerHTML += `<div style="margin-top:8px;padding:10px;background:${treatment.gasto_acreditable ? 'rgba(16,185,129,0.10)' : 'rgba(239,68,68,0.12)'};border-left:3px solid ${treatment.gasto_acreditable ? '#10b981' : '#ef4444'};border-radius:4px;font-size:13px;color:${treatment.gasto_acreditable ? '#d1fae5' : '#fecaca'};">
          ${esc(treatment.microcopy)} <span style="opacity:.7;">(${treatment.legal})</span>
        </div>`;
      }

      // Guardar documento con datos enriquecidos
      ...
      if (output) {
        output.innerHTML = `<div style="color:#ef4444;padding:16px;border:1px solid #ef4444;border-radius:8px;background:rgba(239,68,68,0.1);">
          <div style="font-weight:700;">⚠️ ${error.message}</div>
        </div>`;
      }
    }
  }
